...
void lcdTeks() {
  ...
  if (start) {
    ...
    if (ACMode) {
      lcd.setCursor(12, 0); lcd.print("AC Mode ");
      lcd.setCursor(12, 1); lcd.print("AC:-");
    } else {
      lcd.setCursor(12, 0); lcd.print("Vol Mode");
    }
  }
  //  }
}
...
void lcd_disp(float plateau, float pressure, float peep) {
  ...
  if (start) {
    if (pressure != pressureb) {
      lcd.setCursor(5, 0); lcd.print(pressure, 3);
      pressureb = pressure;
    }
    ...
    if (pressure != pressureb) {
      lcd.setCursor(5, 1); lcd.print(pressure, 3);
      pressureb = pressure;
    }

    if (Q1 != Q1b) {
      lcd.setCursor(5, 4); lcd.print(Q1, 3);
      Q1b = Q1;
    }
