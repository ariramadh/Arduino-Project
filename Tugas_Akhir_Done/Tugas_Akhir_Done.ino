...
String tampil[jmlDHT] = {"Suhu Kotak ","  Suhu Pemanas "};
...
//      Serial.print(i);
    ...
    float input = dhtTFilter(t[0], 0);

    fuzzy->setInput(1, input);  //Memasukan variabel input kedalam input fuzzy 1
    fuzzy->fuzzify(); //Memanggil proses fuzzifikasi
    output = fuzzy->defuzzify(1); //Melakukan proses fuzzifikasi dan keluarannya disimpan kedalam variabel output

    Serial.print("\t||  Input fuzzy\t");
    Serial.print(input);
    Serial.print("\tOutput fuzzy\t");
    Serial.print(output);
    Serial.println(" derajat\t");
    //    Serial.println(T1->getPertinence());

    int nil = analogRead(A0);
    ...
    if (nil < 500) {
      ...
      if (nil < 600) {
        ...
        myservoIn.write(map(nil, 600, 1024, 0, 46));
        myservoOut.write(map(nil, 600, 1024, 4, 74));
      }
    }
...
float dhtHFilter(float rawH, int iteration) {
  float alpha = 0.1;  //Alpha, yaitu nilai seberapa besar tingkat pemfilteran
  float averageH = alpha * rawH + (1 - alpha) * lastHOutput[iteration]; //Persamaan EWMA dalam baris program
  //AverageH adalah hasil, rawH adalah data suhu, lastHOutput adalah data suhu t-1 
  lastHOutput[iteration] = averageH;  //Sebuah variabel //Masukan data suhu yang terfilter kedalam variabel lastHoutput

  averageH = round(averageH * 100) / (float)100;  //Untuk membulatkan nilai AverageH ke 2 angka dibelakang koma

  return averageH;  //Mengembalikan nilai kedalam konstruktor
}
